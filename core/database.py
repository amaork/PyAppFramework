...
class SQLiteDatabase(object):
    ...
    def getRowCount(self, name: str) -> int:
    ...
    def clearTableData(self, name: str, commit: bool = False):
        try:
            self.deleteRecord(name, '', commit)
    ...
    def deleteRecord(self, name: str, condition: str, commit: bool = False) -> bool:
        """Delete records from table, when conditions matched

        :param name: table name
        :param condition: condition
        :param commit: commit delete
        :return: error raise an exception
        """
        try:
            row_count = self.getRowCount(name)
            condition = f' WHERE {condition}' if condition else ''
            self._cursor.execute(f'DELETE FROM {name}{condition};')
        except sqlite3.DatabaseError as error:
            raise SQLiteDatabaseError("Delete error:{}".format(error))
        else:
            return row_count != self.getRowCount(name)
        finally:
            self.commit(commit)
